...
try {
  ...
  $pauseCleared = $false
  if (-not ($PreservePause -and $pauseWasSet)) {
  ...
  try {
    ...
    $verifyMode = if ($PreservePause -and $pauseWasSet) { '--verify-manager' } else { '--verify' }
    $verifyOutput = @(& $node.Path $Injector $verifyMode --port $Port --browser-id $cdpIdentity.BrowserId `
