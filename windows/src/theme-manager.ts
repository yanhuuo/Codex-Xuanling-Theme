...
(() => {
  ...
  const closeButton = (kind, label = "关闭") =>
    `<button class="dtm-close-hit" type="button" data-dtm-close="${escapeHtml(kind)}" aria-label="${escapeHtml(label)}">${closeSvg}</button>`;
  ...
  const runCloseAction = (closeTarget: HTMLElement) => {
    ...
    if (closeKind === "manager" || closeTarget.hasAttribute("data-manager-close")) { hide(); return true; }
    if (closeKind === "localTheme" || closeTarget.hasAttribute("data-local-theme-close")) { localModalOpen = false; resetLocalDraft(); render(); return true; }
    if (closeKind === "themePet" || closeTarget.hasAttribute("data-theme-pet-close")) { petPickerTheme = null; render(); return true; }
    if (closeKind === "themeImages" || closeTarget.hasAttribute("data-theme-images-close")) { imageSettingsTheme = null; imageSettingsError = ""; imageSettingsFiles = []; render(); return true; }
    return false;
  };
  ...
  const onPanelKeydown = (event: KeyboardEvent) => {
    ...
    const closeTarget = (event.target as Element | null)?.closest<HTMLElement>(".dtm-close-hit[data-dtm-close],.dtm-close-hit[data-manager-close],.dtm-close-hit[data-local-theme-close],.dtm-close-hit[data-theme-pet-close],.dtm-close-hit[data-theme-images-close]");
  ...
  const show = async (button, nav) => {
    ...
    if (!panel) { panel = document.createElement("section"); panel.id = PANEL_ID; panel.setAttribute("aria-label", "主题"); panel.addEventListener("click", onPanelClick, true); panel.addEventListener("keydown", onPanelKeydown); panel.addEventListener("change", onPanelChange); panel.addEventListener("input", onPanelInput); document.body.appendChild(panel); }
    positionPanel(panel, nav); panel.hidden = false; panel.innerHTML = `<div class="dtm-wrap"><div class="dtm-loading-card"><div class="dtm-loading-copy"><span class="dtm-loading-spinner" aria-hidden="true"></span><span>正在读取主题…</span></div>${closeButton("manager")}</div></div>`;
    try {
