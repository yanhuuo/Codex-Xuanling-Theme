...
type DreamThemeManagerState = {
  paused: boolean;
  hotReload: boolean;
  active?: { id: string; name: string };
  themes: DreamThemeSummary[];
  bundledThemes: DreamThemeSummary[];
  libraries: Array<{ id: string; type: string; label: string; location: string }>;
  pets: Array<Record<string, unknown>>;
  selectedPet?: string | null;
  canEnableActive: boolean;
};
...
(() => {
  ...
  const themePreviewCache = new Map<string, Promise<string>>();
  const petPreviewCache = new Map<string, Promise<string>>();
  const themePreviewDataUrl = (scope: string, key: string) => {
    const normalizedScope = scope === "bundled" ? "bundled" : "installed";
    const normalizedKey = String(key || "").trim();
    if (!normalizedKey || normalizedKey.length > 160 || /[\\/:*?"<>]|\.\./.test(normalizedKey)) return Promise.resolve("");
    const cacheKey = `${normalizedScope}:${normalizedKey}`;
    if (!themePreviewCache.has(cacheKey)) {
      themePreviewCache.set(cacheKey, call<{ key: string; scope: string; preview?: unknown }>("getThemePreview", { scope: normalizedScope, key: normalizedKey }).then((result) => safePreview(result?.preview)).catch(() => ""));
    }
    return themePreviewCache.get(cacheKey);
  };
  const petThumbCache = new Map<string, Promise<string>>();
  const petPreviewDataUrl = (petId: string) => {
    const normalizedPetId = String(petId || "").trim();
    if (!/^[A-Za-z0-9._-]{1,80}$/.test(normalizedPetId)) return Promise.resolve("");
    if (!petPreviewCache.has(normalizedPetId)) {
      petPreviewCache.set(normalizedPetId, call<{ petId: string; preview?: unknown }>("getPetPreview", { petId: normalizedPetId }).then((result) => safePreview(result?.preview)).catch(() => ""));
    }
    return petPreviewCache.get(normalizedPetId);
  };
  const petThumbDataUrl = (preview: string) => {
  ...
  const applyThemePreviewImages = (panel: HTMLElement) => {
    panel.querySelectorAll<HTMLImageElement>("[data-dtm-theme-preview-key]").forEach((image) => {
      ...
      themePreviewDataUrl(scope, key).then((preview) => {
        if (!preview || !image.isConnected) return;
        image.src = preview;
        image.hidden = false;
  ...
  let localIconsJsonFile: File | null = null;
  let petPickerTheme: DreamThemeSummary | null = null;
  ...
  let localNewIconKey = "";
  const iconLibraryLabels = new Map<string, string>();
  ...
  const preserveLocalTextFields = () => {
    ...
    localThemeName = panel?.querySelector<HTMLInputElement>("[data-local-theme-name]")?.value || localThemeName;
    localBaseKey = panel?.querySelector<HTMLSelectElement>("[data-local-theme-base]")?.value || localBaseKey;
    localAccent = panel?.querySelector<HTMLInputElement>("[data-local-theme-accent]")?.value || localAccent;
    localDetail = panel?.querySelector<HTMLInputElement>("[data-local-theme-detail]")?.value || localDetail;
    localSendBase = panel?.querySelector<HTMLInputElement>("[data-local-theme-send-base]")?.value || localSendBase;
    localProcessingBase = panel?.querySelector<HTMLInputElement>("[data-local-theme-processing-base]")?.value || localProcessingBase;
    localNewIconKey = panel?.querySelector<HTMLInputElement>("[data-local-new-icon-key]")?.value || localNewIconKey;
  };
  ...
  const setLocalColorValue = (key, value) => {
    if (key === "accent") localAccent = value;
    else if (key === "detail") localDetail = value;
    else if (key === "sendBase") localSendBase = value;
    else if (key === "processingBase") localProcessingBase = value;
  };
  ...
  const splitImagePosition = (value) => {
    ...
    if (!normalized || normalized === "auto") return { x: "auto", y: "auto" };
    const parts = normalized.split(/\s+/);
    let x = parts.find((part) => part === "left" || part === "right" || part === "center") || "center";
    let y = parts.find((part) => part === "top" || part === "bottom") || "";
    if (!y && parts.includes("center")) y = "center";
    if (normalized === "top" || normalized === "bottom") { x = "center"; y = normalized; }
    if (normalized === "left" || normalized === "right") { x = normalized; y = "center"; }
    return { x, y: y || "center" };
  };
  ...
  const localCreatorCardHtml = () =>
    `<button class="dtm-card dtm-create-card" type="button" data-local-theme-open><div class="dtm-preview">${palette}</div><div class="dtm-card-body"><div class="dtm-card-line"><div class="dtm-card-main"><div class="dtm-title">本地自定义主题</div><p>选择背景与图标，另存到本机</p></div><span class="dtm-button dtm-button-primary">设置</span></div></div></button>`;
  ...
  const petPickerDialogHtml = () => {
    ...
    return `<div class="dtm-modal-layer" role="presentation"><button class="dtm-modal-backdrop" type="button" data-theme-pet-close aria-label="关闭弹窗"></button><section class="dtm-dialog" role="dialog" aria-modal="true" aria-labelledby="dtm-pet-title"><div class="dtm-dialog-head"><div><h2 id="dtm-pet-title">编辑主题宠物</h2><p>${escapeHtml(petPickerTheme.name)} 的宠物绑定会写入这个主题包配置。</p></div><button class="dtm-close-hit" type="button" data-theme-pet-close aria-label="??"></button></div><div class="dtm-dialog-body">${pets ? `<div class="dtm-grid">${pets}</div>` : '<div class="dtm-empty">没有发现有效的 Codex v2 宠物包。</div>'}</div><div class="dtm-dialog-actions"><button class="dtm-button ${petPickerTheme.petId ? "dtm-button-danger" : ""}" type="button" data-theme-pet-clear="${escapeHtml(petPickerTheme.key)}" ${petPickerTheme.petId ? "" : "disabled"}>解除绑定</button><button class="dtm-button" type="button" data-theme-pet-close>取消</button></div></section></div>`;
  };
  ...
  const render = () => {
    ...
    const baseOptions = (state.bundledThemes || []).map((theme) =>
      `<option value="${escapeHtml(theme.key)}" ${theme.key === localBaseKey ? "selected" : ""}>${escapeHtml(theme.name)}</option>`).join("");
  ...
  const closeButtonFromPoint = (event: MouseEvent | PointerEvent) => {
    const panel = document.getElementById(PANEL_ID);
    if (!panel) return null;
    for (const button of Array.from(panel.querySelectorAll<HTMLElement>(".dtm-close-hit[data-manager-close],.dtm-close-hit[data-local-theme-close],.dtm-close-hit[data-theme-pet-close],.dtm-close-hit[data-theme-images-close]"))) {
      const rect = button.getBoundingClientRect();
      if (event.clientX >= rect.left && event.clientX <= rect.right && event.clientY >= rect.top && event.clientY <= rect.bottom) return button;
    }
    return null;
  };
  const runCloseAction = (closeTarget: HTMLElement) => {
    if (closeTarget.hasAttribute("data-manager-close")) { hide(); return true; }
    if (closeTarget.hasAttribute("data-local-theme-close")) { localModalOpen = false; resetLocalDraft(); render(); return true; }
    if (closeTarget.hasAttribute("data-theme-pet-close")) { petPickerTheme = null; render(); return true; }
    if (closeTarget.hasAttribute("data-theme-images-close")) { imageSettingsTheme = null; imageSettingsError = ""; imageSettingsFiles = []; render(); return true; }
    return false;
  };
  const onPanelPointerMove = (event: PointerEvent) => {
    ...
    panel.querySelectorAll<HTMLElement>(".dtm-close-hit").forEach((button) => {
      button.classList.toggle("dtm-close-hover", button === hovered);
    });
  };
  const onPanelPointerLeave = () => {
    document.querySelectorAll<HTMLElement>(`#${PANEL_ID} .dtm-close-hit.dtm-close-hover`).forEach((button) => button.classList.remove("dtm-close-hover"));
  };
  const onPanelClick = (event: MouseEvent) => {
    ...
    if (pointCloseTarget) {
      ...
      if (runCloseAction(pointCloseTarget)) return;
    }
    const element = event.target as Element | null;
    const closeTarget = element?.closest<HTMLElement>("[data-manager-close],[data-local-theme-close],[data-theme-pet-close],[data-theme-images-close]");
    ...
    if (target.dataset.managerTab) { activeTab = target.dataset.managerTab === "pets" ? "pets" : "themes"; render(); }
      ...
      const fit = panel.querySelector<HTMLSelectElement>("[data-theme-image-fit]")?.value || "cover";
      const positionX = panel.querySelector<HTMLSelectElement>("[data-theme-image-position-x]")?.value || "auto";
      const positionY = panel.querySelector<HTMLSelectElement>("[data-theme-image-position-y]")?.value || "auto";
      const position = imagePositionValue(positionX, positionY);
      const repeat = panel.querySelector<HTMLSelectElement>("[data-theme-image-repeat]")?.value || "no-repeat";
      ...
      const panel = document.getElementById(PANEL_ID);
      const name = panel.querySelector<HTMLInputElement>("[data-local-theme-name]")?.value || "";
      const baseKey = panel.querySelector<HTMLSelectElement>("[data-local-theme-base]")?.value || "";
      const imageFile = localImageFile;
      const iconsJsonFile = localIconsJsonFile;
      act(async () => {
        if (!imageFile) throw new Error("请先选择一张背景图片");
        const iconOverrides = {};
        for (const [key, svg] of localIconSvgs.entries()) iconOverrides[key] = svg;
  ...
  const onPanelKeydown = (event: KeyboardEvent) => {
    ...
    const closeTarget = (event.target as Element | null)?.closest<HTMLElement>(".dtm-close-hit[data-manager-close],.dtm-close-hit[data-local-theme-close],.dtm-close-hit[data-theme-pet-close],.dtm-close-hit[data-theme-images-close]");
    if (!closeTarget) return;
    event.preventDefault();
    closeTarget.click();
  };
  const onPanelChange = async (event: Event) => {
    const input = event.target as HTMLInputElement | HTMLSelectElement | null;
    if (!input) return;
    if (input.matches("[data-local-theme-name]")) localThemeName = input.value;
    ...
    else if (input.matches("[data-local-theme-send-base]")) localSendBase = input.value;
    else if (input.matches("[data-local-theme-processing-base]")) localProcessingBase = input.value;
    else if (input.matches("[data-local-new-icon-key]")) localNewIconKey = input.value;
    else if (input.matches("[data-local-icon-library]")) {
      preserveLocalTextFields();
      const key = (input as HTMLSelectElement).dataset.localIconLibrary || "";
      ...
      localImageFile = (input as HTMLInputElement).files?.[0] || null;
      const label = document.querySelector<HTMLElement>(`#${PANEL_ID} [data-local-theme-image-name]`);
      if (label) label.textContent = localImageFile?.name || "支持 PNG、JPG、WebP、GIF，最大 16 MB";
    } else if (input.matches("[data-theme-images-file]")) {
      imageSettingsFiles = Array.from((input as HTMLInputElement).files || []).slice(0, 8);
      imageSettingsError = "";
      render();
      ...
      try {
        ...
        const source = parsed?.icons && typeof parsed.icons === "object" && !Array.isArray(parsed.icons)
          ? parsed.icons : parsed;
        if (!source || typeof source !== "object" || Array.isArray(source)) throw new Error("JSON 必须是图标对象");
        const entries = Object.entries(source);
        if (entries.length > 128) throw new Error("图标数量不能超过 128 个");
        const imported = new Map<string, string>();
        for (const [key, value] of entries) {
          const svg = safeIconSvg(value);
          if (!/^[A-Za-z][A-Za-z0-9_-]{0,39}$/.test(key) || !svg) throw new Error(`图标 ${key} 不是有效 SVG`);
          imported.set(key, svg);
        }
        localIconsJsonFile = file;
        localJsonIcons.clear();
        for (const [key, svg] of imported) localJsonIcons.set(key, svg);
        localDraftError = "";
      } catch (error) {
        localIconsJsonFile = null;
        localJsonIcons.clear();
      ...
      if (!file) return;
      preserveLocalTextFields();
      const key = (input as HTMLInputElement).dataset.localIconFile || "";
  ...
  const show = async (button, nav) => {
    ...
    positionPanel(panel, nav); panel.hidden = false; panel.innerHTML = '<div class="dtm-wrap"><div class="dtm-loading-card"><div class="dtm-loading-copy"><span class="dtm-loading-spinner" aria-hidden="true"></span><span>正在读取主题…</span></div><button class="dtm-close-hit" type="button" data-manager-close aria-label="??"></button></div></div>';
    try {
      ...
      if (!showing || token !== showSequence) return;
      panel.innerHTML = `<div class="dtm-wrap"><div class="dtm-head"><div><h1>主题</h1></div><button class="dtm-close-hit" type="button" data-manager-close aria-label="??"></button></div><div class="dtm-empty">${escapeHtml(error.message || error)}</div></div>`;
    }
  };
  const hide = () => { showSequence += 1; showing = false; localModalOpen = false; resetLocalDraft(); const panel = document.getElementById(PANEL_ID); if (panel) panel.hidden = true; };
  const ensure = () => {
    const nav = document.querySelector('nav[aria-label="设置"],nav[aria-label="Settings"]');
    if (!nav) { hide(); return false; }
    let button = nav.querySelector<HTMLButtonElement>(`button[data-settings-panel-slug="${NAV_SLUG}"]`);
    if (button && button.dataset.dreamThemeManagerVersion !== VERSION) {
      button.remove();
      button = null;
    }
    if (!button) {
      const appearance = nav.querySelector<HTMLButtonElement>('button[data-settings-panel-slug="appearance"],button[aria-label="外观"],button[aria-label="Appearance"]');
      if (!appearance) return false;
      button = appearance.cloneNode(true) as HTMLButtonElement;
  ...
  const onDocumentClick = (event: MouseEvent) => {
    const settingsButton = (event.target as Element | null)?.closest?.<HTMLButtonElement>('button[data-settings-panel-slug]');
