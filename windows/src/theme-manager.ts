...
(() => {
  ...
  const onPanelClick = (event: MouseEvent) => {
    ...
    if (target.dataset.managerTab) { activeTab = target.dataset.managerTab === "pets" ? "pets" : "themes"; render(); }
      ...
      const position = panel.querySelector<HTMLSelectElement>("[data-theme-image-position]")?.value || "auto";
  ...
  const show = async (button, nav) => {
    ...
    positionPanel(panel, nav); panel.hidden = false; panel.innerHTML = '<div class="dtm-wrap"><div class="dtm-loading-card"><div class="dtm-loading-copy"><span class="dtm-loading-spinner" aria-hidden="true"></span><span>正在读取主题…</span></div><button class="dtm-close-hit" type="button" data-manager-close aria-label="Close"></button></div></div>';
    try {
