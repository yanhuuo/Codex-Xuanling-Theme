...
$script:DreamSkinManagedLightCodeTheme = 'appearanceLightCodeThemeId = "github"'
$script:DreamSkinManagedLightChromeTheme = 'appearanceLightChromeTheme = { accent = "#2B8FBF", contrast = 52, fonts = { code = "Cascadia Code", ui = "Microsoft YaHei UI" }, ink = "#172B38", opaqueWindows = false, semanticColors = { diffAdded = "#269B7D", diffRemoved = "#C04F72", skill = "#7658BD" }, surface = "#F2F8FA" }'
...
function Write-DreamSkinBytesAtomically {
  ...
  try {
    ...
    if ([System.IO.File]::Exists($fullPath)) {
      ...
      # for File.Replace on some systems, so use a same-directory backup and
      # remove it after the atomic replacement succeeds.
...
function Assert-DreamSkinTomlLineEditingSafe {
  ...
  # In .NET's multiline mode, `$` is positioned before `\n` but after the
  # preceding `\r`; explicitly consume that carriage return for CRLF files.
...
function Restore-DreamSkinBaseTheme {
  ...
  # If restoring only the managed keys recreated the exact backup plus one
  # trailing newline, prefer the byte-equivalent backup shape. This keeps
