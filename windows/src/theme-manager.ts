...
(() => {
  ...
  let catalog: DreamThemeCatalog | null = null;
  let message = "";
  let showing = false;
  ...
  const render = () => {
    ...
    if (localModalOpen && !localBaseKey) localBaseKey = state.bundledThemes?.[0]?.key || "";
  ...
  const onPanelClick = (event: MouseEvent) => {
    ...
    if (target.dataset.managerTab) { activeTab = target.dataset.managerTab === "pets" ? "pets" : "themes"; render(); }
    else if (target.hasAttribute("data-local-theme-open")) { resetLocalDraft(); localModalOpen = true; render(); }
    else if (target.hasAttribute("data-local-theme-close")) { localModalOpen = false; resetLocalDraft(); render(); }
      ...
      act(async () => {
        ...
        const iconOverrides = {};
        for (const [key, file] of iconFiles) iconOverrides[key] = await file.text();
      ...
      const panel = document.getElementById(PANEL_ID); const location = panel.querySelector<HTMLInputElement>("[data-repository-location]")?.value || ""; const label = panel.querySelector<HTMLInputElement>("[data-repository-label]")?.value || "";
      act(() => call("addRepository", { location, label }), "GitHub 主题仓库已添加");
    }
    else if (target.hasAttribute("data-library-add")) {
      const panel = document.getElementById(PANEL_ID); const location = panel.querySelector<HTMLInputElement>("[data-library-location]")?.value || ""; const label = panel.querySelector<HTMLInputElement>("[data-library-label]")?.value || "";
    ...
    else if (target.dataset.libraryOpen) act(async () => { catalog = await call<DreamThemeCatalog>("getCatalog", { id: target.dataset.libraryOpen }); return call("getState"); }, "主题库读取完成");
  ...
  const onPanelChange = async (event: Event) => {
    ...
    if (input.matches("[data-local-theme-name]")) localThemeName = input.value;
      ...
      if (label) label.textContent = localImageFile?.name || "支持 PNG、JPG、WebP，最大 16 MB";
  ...
  const show = async (button, nav) => {
    ...
    if (!panel) { panel = document.createElement("section"); panel.id = PANEL_ID; panel.setAttribute("aria-label", "主题"); panel.addEventListener("click", onPanelClick); panel.addEventListener("change", onPanelChange); document.body.appendChild(panel); }
  ...
  const hide = () => { showing = false; localModalOpen = false; resetLocalDraft(); const panel = document.getElementById(PANEL_ID); if (panel) panel.hidden = true; };
  const ensure = () => {
