(() => {
  ...
  const petHtml = (pet) => {
    const active = state?.petAssociation === pet.id;
    return `<article class="dtm-card"><div class="dtm-preview">${bird}</div><div class="dtm-card-body"><div class="dtm-title"><span>${escapeHtml(pet.displayName)}</span><span class="dtm-chip">宠物 v${escapeHtml(pet.spriteVersionNumber)}</span></div>${pet.description ? `<p style="margin:-4px 0 10px">${escapeHtml(pet.description)}</p>` : ""}<button class="dtm-button ${active ? "" : "dtm-button-primary"}" data-pet-associate="${escapeHtml(pet.id)}">${active ? "已关联当前主题" : "关联到当前主题"}</button></div></article>`;
  };
  const catalogHtml = () => {
    if (!catalog) return "";
  ...
  const render = () => {
    ...
    panel.innerHTML = `<div class="dtm-wrap"><div class="dtm-head"><div><h1>主题</h1><p>每个主题都是独立文件夹，完整包含 theme.json、theme.css、theme.js 和图片资源；切换时整包切换，互不污染。</p></div><div class="dtm-status"><span class="dtm-dot"></span>${state.paused ? "官方外观" : `当前：${escapeHtml(state.active?.name || "主题")}`} · ${state.hotReload ? "热重载已开启" : "自动刷新"}</div></div><div class="dtm-row"><button class="dtm-button ${state.paused ? "dtm-button-primary" : "dtm-button-danger"}" data-official>${state.paused ? "重新启用当前主题" : "还原官方外观"}</button><button class="dtm-button" data-refresh>立即刷新</button><p>还原只暂停主题注入，不删除已安装主题、宠物关联和主题库配置。</p></div><section class="dtm-section"><h2>已安装主题</h2>${themes ? `<div class="dtm-grid">${themes}</div>` : '<div class="dtm-empty">暂无已安装主题。</div>'}</section><section class="dtm-section"><h2>配套宠物</h2><p style="margin-bottom:12px">这里读取 Codex 原生 <code>~/.codex/pets</code> v2 宠物包，并记录每个主题的配套宠物。宠物动画仍由 Codex 原生宠物窗口渲染。</p>${pets ? `<div class="dtm-grid">${pets}</div><div class="dtm-row" style="margin-top:12px"><button class="dtm-button ${state.petAssociation ? "dtm-button-danger" : ""}" data-pet-clear ${state.petAssociation ? "" : "disabled"}>取消当前关联</button></div>` : '<div class="dtm-empty">没有发现有效的 Codex v2 宠物包。</div>'}</section><section class="dtm-section"><h2>从 GitHub 仓库安装</h2><p style="margin-bottom:12px">公开仓库根目录放置 <code>theme-library.json</code> 后，可直接添加仓库并读取主题；无需手填 Raw 地址。</p><div class="dtm-row"><div class="dtm-grow"><input data-repository-location placeholder="https://github.com/owner/repository"></div><div style="min-width:150px"><input data-repository-label placeholder="名称（可选）"></div><button class="dtm-button dtm-button-primary" data-repository-add>添加仓库</button></div></section><section class="dtm-section"><h2>主题库</h2><p style="margin-bottom:12px">也支持本地主题包目录，或 HTTPS 远程索引。主题包包含可执行渲染脚本，只添加你信任的来源。远程索引格式为 <code>{ "themes": [{ "id", "name", "themeUrl" }] }</code>。</p><div class="dtm-row"><div class="dtm-grow"><input data-library-location placeholder="本地主题包目录或 https://…/index.json"></div><div style="min-width:150px"><input data-library-label placeholder="名称（可选）"></div><button class="dtm-button dtm-button-primary" data-library-add>添加主题库</button></div><div style="margin-top:12px">${libraries || '<div class="dtm-empty">尚未配置主题库。</div>'}</div></section>${catalogHtml()}${message ? `<div class="dtm-message">${escapeHtml(message)}</div>` : ""}</div>`;
  };
  ...
  const onPanelClick = (event) => {
    ...
    else if (target.dataset.petAssociate) act(() => call("associatePet", { petId: target.dataset.petAssociate }), "配套宠物已关联");
    else if (target.hasAttribute("data-pet-clear")) act(() => call("associatePet", { petId: "" }), "宠物关联已取消");
