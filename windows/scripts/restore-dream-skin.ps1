...
try {
  ...
  try {
    ...
    Remove-Item -LiteralPath (Join-Path $StateRoot 'guard.enabled') -Force -ErrorAction SilentlyContinue
    if ($Uninstall) {
      ...
      @(
        ...
        (Join-Path ([Environment]::GetFolderPath('Startup')) 'Codex Dream Skin Guard.lnk')
