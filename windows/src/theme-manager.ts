...
(() => {
  ...
  const runCloseAction = (closeTarget: HTMLElement) => {
    ...
    if (closeKind === "themeImages" || closeTarget.hasAttribute("data-theme-images-close")) { imageSettingsTheme = null; imageSettingsLoading = false; imageSettingsError = ""; imageSettingsFiles = []; render(); return true; }
    return false;
  };
  const onPanelClick = (event: MouseEvent) => {
    const element = event.target as Element | null;
    if (element?.id === PANEL_ID) { hide(); return; }
    ...
    if (target.dataset.managerTab) { activeTab = target.dataset.managerTab === "pets" ? "pets" : "themes"; render(); }
      ...
      if (imageSettingsTheme?.key) {
        ...
        call<DreamThemeSummary & { key: string }>("getThemeImages", { key }).then((result) => {
          if (!result || imageSettingsTheme?.key !== key) return;
          imageSettingsTheme = {
            ...imageSettingsTheme,
            ...result,
            key: result.key || key,
          ...
          if (imageSettingsTheme?.key !== key) return;
          imageSettingsLoading = false;
  ...
  const show = async () => {
    ...
    showing = true; message = ""; getStyle(); setTriggerExpanded(true);
  ...
  const onDocumentClick = (event: MouseEvent) => {
    ...
    const settingsButton = element?.closest?.<HTMLButtonElement>('button[data-settings-panel-slug]');
  ...
  const onDocumentPointerDown = (event: PointerEvent) => {
    ...
    const closeTarget = element?.closest?.<HTMLElement>("[data-dtm-close],[data-manager-close],[data-local-theme-close],[data-theme-pet-close],[data-theme-images-close]");
    const panel = document.getElementById(PANEL_ID);
    if (!closeTarget || !panel?.contains(closeTarget)) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    runCloseAction(closeTarget);
  };
